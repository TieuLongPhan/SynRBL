...
def barplot_accuracy_comparison_2x2(
    ...
    fig, axs = plt.subplots(2, 2, figsize=(16, 14))  # Adjusted for a 2x2 layout
    ...
    for i, (df, ax) in enumerate(zip(dfs, axs)):
        ...
        ax.spines['left'].set_color('grey')
        ax.spines['bottom'].set_color('grey')
        ax.set_title(title_names[i], fontsize=18, fontweight='bold')
        ax.text(-0.05, 1.05, labels[i], transform=ax.transAxes, size=18, weight='bold')
    ...
    fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 0.03), fontsize=14, ncol=2)
