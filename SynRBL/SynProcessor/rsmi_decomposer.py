...
class RSMIDecomposer:
    ...
    """
    atomic_symbols = {
            0: 'Q', 1: 'H', 2: 'He', 3: 'Li', 4: 'Be', 5: 'B', 6: 'C', 7: 'N', 8: 'O', 9: 'F', 10: 'Ne',
    ...
    def data_decomposer(self) -> Tuple[List[Dict[str, int]], List[Dict[str, int]]]:
        ...
        Returns:
        tuple of lists: Two lists containing dictionaries of atomic compositions for reactants and products.
        ...
        if self.parallel:
            react_dict = Parallel(n_jobs=self.n_jobs, verbose=self.verbose)(delayed(RSMIDecomposer.decompose)(smiles) for smiles in reactants)
            prods_dict = Parallel(n_jobs=self.n_jobs, verbose=self.verbose)(delayed(RSMIDecomposer.decompose)(smiles) for smiles in products)
        else:
    ...
    def decompose(smiles: str) -> Dict[str, int]:
        ...
        Returns:
        dict
            A dictionary with atomic symbols as keys and counts as values. Charge is represented as 'Q'.
