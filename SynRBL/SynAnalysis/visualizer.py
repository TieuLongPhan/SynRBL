...
def barplot_accuracy_comparison_2x2(
    dfs: List[pd.DataFrame], 
    show_values: bool = False, 
    title_names: List[str] = ['Rule-based', 'MCS-based', 'Overall', 'Confidence-constraint'],  # Adjusted for a 2x2 layout
    save_path: Optional[str] = None
) -> None:
    """
    ...
    for i, (df, ax) in enumerate(zip(dfs, axs)):
        ...
        bar_width, index = 0.35, np.arange(len(datasets))
        bars1 = ax.bar(index - bar_width/2, success_rate*100, bar_width, yerr=error_success.T*100, 
                       zorder=2, label='Success Rate', color=color_for_success_rate, error_kw={'ecolor':'black', 'elinewidth':1})
        bars2 = ax.bar(index + bar_width/2, accuracy*100, bar_width, yerr=error_accuracy.T*100, 
                       zorder=2, label='Accuracy', color=color_for_accuracy, error_kw={'ecolor':'black', 'elinewidth':1})
        ...
        if show_values:
            for bars in [bars1, bars2]:
                for bar in bars:
                    height = bar.get_height()
                    ax.annotate(f'{height:.2f}%', xy=(bar.get_x() + bar.get_width() / 2, height*1.02), xytext=(0, 3), textcoords="offset points", ha='center', va='bottom', fontsize=8)
