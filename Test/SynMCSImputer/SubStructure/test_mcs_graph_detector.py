...
class TestMCSMissingGraphAnalyzer(unittest.TestCase):
    ...
    def test_get_smiles(self):
        reactant_smiles, product_smiles = MCSMissingGraphAnalyzer.get_smiles(self.reaction_dict)
    ...
    def test_fit_balanced(self):
        ...
        mcs_list, sorted_parents, reactant_mol_list, product_mol = MCSMissingGraphAnalyzer.fit(self.reaction_dict)
    ...
    def test_fit_unbalanced_products(self):
        ...
        mcs_list, sorted_parents, reactant_mol_list, product_mol = MCSMissingGraphAnalyzer.fit(self.reaction_dict_unbalanced)
    ...
    def test_fit_unbalanced_reactants(self):
        ...
        mcs_list_unbalanced, sorted_parents_unbalanced, product_mol_list_unbalanced, reactant_mol_unbalanced = MCSMissingGraphAnalyzer.fit(self.reaction_dict_unbalanced_reactants)

        self.assertTrue(all(isinstance(mol, Chem.Mol) for mol in product_mol_list_unbalanced))
        self.assertIsInstance(reactant_mol_unbalanced, Chem.Mol)
        self.assertTrue(len(mcs_list_unbalanced) == 1)
        self.assertEqual(Chem.MolToSmarts(mcs_list_unbalanced[0]),'[#8]=&!@[#6]-&!@[#6]1:&@[#6]:&@[#6]:&@[#6]:&@[#6]:&@[#6]:&@1')

    def test_IterativeMCSReactionPairs(self):
        reactant_smiles, product_smiles = MCSMissingGraphAnalyzer.get_smiles(self.reaction_dict_unbalanced)
        reactant_mols = [MCSMissingGraphAnalyzer.convert_smiles_to_molecule(smiles) for smiles in reactant_smiles.split('.')]
        product_mol = MCSMissingGraphAnalyzer.convert_smiles_to_molecule(product_smiles)

        # Test with MCIS
        mcs_list_MCIS, sorted_reactants_MCIS = MCSMissingGraphAnalyzer.IterativeMCSReactionPairs(
            reactant_mols, product_mol, self.params_MCIS, method='MCIS', sort='MCIS', remove_substructure=True)
        self.assertTrue(all(isinstance(mol, Chem.Mol) for mol in mcs_list_MCIS))

        # Test with MCES
        reactant_mols = [Chem.MolFromSmiles('CN(C)c1ccc(CC(=O)NCCCCCCCCCCNC23CC4CC(C2)CC(C3)C4)cc1')]
        product_mol = Chem.MolFromSmiles('CN(C)c1ccc(CC(=O)NCCCCCCCCCCCCNC23CC4CC(C2)CC(C3)C4)cc1')

        mcs_list_MCES, sorted_reactants_MCES = MCSMissingGraphAnalyzer.IterativeMCSReactionPairs(
            reactant_mols, product_mol, self.params_MCES, method='MCES', sort='MCES', remove_substructure=True)
