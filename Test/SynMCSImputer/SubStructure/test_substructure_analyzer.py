...
class TestSubstructureAnalyzer(unittest.TestCase):
    ...
    def test_remove_substructure_atoms(self):
        ...
        fragment_count = self.analyzer.remove_substructure_atoms(self.reactant, substructures[0])
        self.assertEqual(fragment_count, 3)  # break wrong

        fragment_count = self.analyzer.remove_substructure_atoms(self.reactant, substructures[1])
    ...
    def test_sort_substructures_by_fragment_count(self):
        ...
        sorted_substructures = self.analyzer.sort_substructures_by_fragment_count(substructures, fragment_counts)
        print(sorted_substructures)
        self.assertEqual(sorted_substructures, [(4, 3, 5, 6, 7, 8, 9, 10), (1, 3, 5, 6, 7, 8, 9, 10)])  # Should be sorted by fragment count
    ...
    def test_identify_optimal_substructure(self):
        ...
        optimal_substructure = self.analyzer.identify_optimal_substructure(self.reactant, self.product)
        print(optimal_substructure)
        self.assertEqual(optimal_substructure, (4, 3, 5, 6, 7, 8, 9, 10))
        self.assertNotEqual(optimal_substructure,(1, 3, 5, 6, 7, 8, 9, 10))  # Assuming this is the correct substructure
