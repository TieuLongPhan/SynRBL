...
from SynRBL.SynChemImputer.peroxide_imputer import PeroxidGroupImputer  # Replace with the actual module name
...
class TestPeroxidGroupImputer(unittest.TestCase):
    ...
    def test_fix_peroxid_group(self):
        ...
        expected_result = {'reactions': 'COOC.C>>C.CO.CO'}  # Assuming this is the expected updated reaction
    ...
    def test_fix_peracid_group(self):
        ...
        expected_result = {'reactions': 'C=C.C1=CC(=CC(=C1)Cl)C(=O)OO>>O1CC1.O=C(O)c1cccc(Cl)c1'}  # Updated reaction
    ...
    def test_fix_peracid_group_not_impute(self):
        ...
        reactions_dict = [{'reactions': 'C=C.C1=CC(=CC(=C1)Cl)C(=O)OO>>O=C(O)c1cccc(Cl)c1.O1CC1'}]
        expected_result = {'reactions': 'C=C.C1=CC(=CC(=C1)Cl)C(=O)OO>>O=C(O)c1cccc(Cl)c1.O1CC1'}  # No change
