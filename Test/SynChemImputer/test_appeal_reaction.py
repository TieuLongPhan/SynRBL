...
from SynRBL.SynChemImputer.appel_reaction import AppelReaction  # Make sure to import your class correctly
...
class TestAppelReaction(unittest.TestCase):
    ...
    def test_check_appel_reaction(self):
        ...
        self.assertTrue(AppelReaction.check_appel_reaction('ClC(Cl)(Cl)Cl.CCO'))  # TCM with ethanol
    ...
    def test_check_missing_reagent(self):
        ...
        self.assertFalse(AppelReaction.check_missing_reagent('ClC(Cl)(Cl)Cl.CCO.c1ccccc1P(c2ccccc2)c3ccccc3'))
    ...
    def test_check_missing_products(self):
        ...
        self.assertFalse(AppelReaction.check_missing_products('CCCl.O=P(c1ccccc1)(c2ccccc2)c3ccccc3'))

    def test_fit(self):
        # Test the fit function with various scenarios
        appel_reactor = AppelReaction()
        reaction_dict = {
            "rmsi_col": "CCO.ClC(Cl)(Cl)Cl>>CCCl"
        }
        updated_dict = appel_reactor.fit(reaction_dict, "rmsi_col")
        self.assertIn(AppelReaction.TPPO, updated_dict["rmsi_col"])  # Check for TPPO in products
        self.assertIn(AppelReaction.TCM_product, updated_dict["rmsi_col"])  # Check for TCM product
    ...
    def test_fit_with_existing_tp_po(self):
        appel_reactor = AppelReaction()
        reaction_dict = {"rmsi_col": "CCO.ClC(Cl)(Cl)Cl>>CCCl.'O=P(c1ccccc1)(c2ccccc2)c3ccccc3'"}
        updated_dict = appel_reactor.fit(reaction_dict, "rmsi_col")
        self.assertIn(AppelReaction.TPP, updated_dict["rmsi_col"])

    def test_fit_with_existing_tpp(self):
        appel_reactor = AppelReaction()
        reaction_dict = {"rmsi_col": "CCO.ClC(Cl)(Cl)Cl.c1ccccc1P(c2ccccc2)c3ccccc3>>CCCl'"}
        updated_dict = appel_reactor.fit(reaction_dict, "rmsi_col")
        self.assertIn(AppelReaction.TPPO, updated_dict["rmsi_col"])
        self.assertIn(AppelReaction.TCM_product, updated_dict["rmsi_col"])
